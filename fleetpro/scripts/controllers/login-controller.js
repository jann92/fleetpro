app.controller('LoginController', ['$scope', '$localStorage', 'LoginFactory', function ($scope, $localStorage, LoginFactory) {
    ...
    if (hostname == "login.philtracker.com" || hostname == "app2.indotrack.com") {
        $scope.logo_url = null;
    } else {
    ...
    $scope.getSelected = function (data) {
        ...
        $localStorage.imagesUrl_TRR = data.ImagesUrl;
        ...
        $scope.login = function () {
            ...
            if ($("#login-account").parsley().isValid()) {
                ...
                var loginUrl = $localStorage.serverUrl_TRP == "../../" ? "../token" : $localStorage.serverUrl_TRP  + "token" ;
                ...
                $.getJSON("https://jsonip.com?callback=?", function (ip) {
                    ...
                    LoginFactory.login(loginUrl, formData, function (res) {
                        ...
                        if (res.token_type) {
                            ...
                            $localStorage.serverCode = $scope.serverCode;
                            $localStorage.serverUrl = $scope.serverUrl;
                            $localStorage.username = $scope.username;
                            $localStorage.defaultCoordinates = $scope.defaultCoordinates;
                            $localStorage.moduleRoles = [];
                            ...
                            $localStorage.serverCode_TRP = $scope.serverCode;
                            $localStorage.serverUrl_TRP = $scope.serverUrl;
                            $localStorage.username_TRP = $scope.username;
                            $localStorage.defaultCoordinates_TRP = $scope.defaultCoordinates;
                            ...
                            //fleetadmin
                            $localStorage.access_token_TRA = res.access_token;
                            $localStorage.expires_in_TRA = res.expires_in;
                            $localStorage.serverCode_TRA = $scope.serverCode;
                            $localStorage.serverUrl_TRA = $scope.serverUrl;
                            $localStorage.username_TRA = $scope.username;
                            $localStorage.defaultCoordinates_TRA = $scope.defaultCoordinates;
                            $localStorage.moduleRoles_TRA = [];
                            ...
                            //insightpro token
                            localStorage.setItem("currentUser_ISP", JSON.stringify(curUser));

                            //campro token
                            localStorage.setItem("currentUser_CAM", JSON.stringify(curUser));
                            ...
                            LoginFactory.getRolesList(function (res) {
                                ...
                                if (res != null) {
                                    ...
                                    $localStorage.moduleRoles_TRR = res;
                                    $localStorage.moduleRoles = res;

                                }

                                LoginFactory.getParentID(function (data) {
                                    $localStorage.parentID = data.ParentID;
